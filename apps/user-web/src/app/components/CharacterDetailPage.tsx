...
import {
  ArrowLeft, Trophy, Lock, BookOpen, User,
  Flame, Star, Zap, Sparkles, CheckCircle2, Shield,
} from "lucide-react";
    ...
    <div className="space-y-4 max-w-lg">
      ...
      {tab === "character" && (
        <div className="space-y-4">
          ...
          <div className={`bg-card rounded-xl border-2 ${RARITY_BORDER[currentChar.rarity]} p-6`}>
            <div className="flex flex-col items-center gap-3">
              <div className={`p-4 rounded-2xl ${RARITY_BG[currentChar.rarity]}`}>
                <PixelCharacter characterId={currentChar.id} size={128} />
              </div>
              <div className="text-center">
                <div className="flex items-center justify-center gap-2 mb-1">
                  <span className={`text-xs font-bold px-2 py-0.5 rounded-full ${RARITY_BG[currentChar.rarity]} ${RARITY_COLOR[currentChar.rarity]}`}>
                    {RARITY_LABEL[currentChar.rarity]}
                  </span>
                  <span className="text-xs text-muted-foreground">#{currentChar.id}</span>
                </div>
                <p className={`text-xl font-bold ${RARITY_COLOR[currentChar.rarity]}`}>{currentChar.korName}</p>
          ...
          {selectedChar && (
            <div className={`bg-card rounded-xl border ${RARITY_BORDER[selectedChar.rarity]} p-4`}>
              <div className="flex items-center gap-4">
                <div className={`p-2 rounded-xl ${RARITY_BG[selectedChar.rarity]} ${selectedChar.unlockLevel > level ? "grayscale opacity-50" : ""}`}>
                  <PixelSprite type={selectedChar.type} colors={selectedChar.colors} size={56} />
                </div>
                <div className="flex-1 min-w-0">
                ...
                <div className="shrink-0">
                  {selectedChar.unlockLevel > level ? (
                    ...
                    <button
                      onClick={() => handleEquip(selectedChar.id)}
                      disabled={equipping}
